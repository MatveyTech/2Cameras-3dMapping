...
imgname = "video1_frame1.jpeg"          # query image (large scene)
imgname2 = "video1_frame2.jpeg"   # train image (small object)
    ...
    if m1.distance < 0.35 * m2.distance:
        ...
        if(dict1.get(pt1,None) != 1 and dict2.get(pt2,None)!= 1):
            ...
            if(dict1.get(pt1,None) ==1):
                print("duplicate:(" + str(pt1[0]) +"," +str(pt1[1]) + ")")
            if(dict2.get(pt2,None) ==1):
                print("duplicate:(" + str(pt2[0]) +"," +str(pt2[1]) + ")" )
